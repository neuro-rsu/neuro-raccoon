...
const urlRACCOON = import.meta.url;

document.addEventListener('mousedown', (e) => RACCOON.mousePos = new DOMRect(e.pageX, e.pageY));
if (!window.RACCOONRect) {
    window.RACCOONRect = function(element) {
        if (element && element.host) element = element.host;
        const pos = element ? element.getBoundingClientRect() : RACCOON.mousePos;
        return pos ? {
...
export class RaccoonElement extends LitElement {
    constructor() {
        super();
        ...
        this.$url = `${urlRACCOON.replace('lit-raccoon.js', '')}${name}.js`;
        console.log(this.$url);
        console.log(urlRACCOON);

        this.$ulid = this.$ulid || RACCOON.ulid();
        if (this._useInfo) this.$urlInfo = `${urlRACCOON.replace('lit-raccoon.js', '')}/${name}/$info/$info.js`;
    }
    connectedCallback() {
        ...
        if (this.$$ && this.__globals) {
            RACCOON.$$.listen(this.fnGlobals);
            this.__globals.forEach(i => {
                if (RACCOON.$$[i] === undefined) RACCOON.$$[i] = this[i];
                else this[i] = RACCOON.$$[i];
            });
    ...
    disconnectedCallback() {
        ...
        if (RACCOON.$$ && this.__globals) RACCOON.$$.unlisten(this.fnGlobals);
    ...
    _initBus() {
        if (this.$partid || this.$properties.get('$partid') || (!this.$$ && (this.$properties.get('_partid') || this.__saves || !this.$root || this.__locals || this.__globals))) {
            ...
            if (!RACCOON._$$[this._partid]) {
                RACCOON._$$[this._partid] = { _$$: {}, _$$: {} };
                RACCOON._$$[this._partid]._$$ = icaro({});
                RACCOON._$$[this._partid]._$$.update = icaro({ value: 0 });
            }
        }
    }
    ...
    get $$() { return RACCOON._$$?.[this.partid]?.['_$$'] ? RACCOON._$$[this.partid]['_$$'] : undefined }
        ...
        for (const prop of changedProps.keys()) {
            ...
            if (this.__isFirstUpdated) {
                ...
                if (this.$$ && this.__globals && this.__globals.includes(prop))
                    RACCOON.$$[prop] = this[prop];
                // if (RACCOON._changed) console.log('_changed', this.localName, prop, { old: changedProps.get(prop) }, { new: this[prop] });
                // if (RACCOON._$update) ++this.$$.__changed;
            }
            if (this.__notifications && this.__notifications.has(prop)) {
                ...
                //if (RACCOON._notify) console.log('_notify ', this.localName, event, { old: changedProps.get(prop) }, { new: this[prop] });
            }
        }
    }

    $update(property, value) { RACCOON.$update.call(this, property, value) }
    $listen(event, fn) {
        if (!event) return;
        RACCOON.$$[event] = RACCOON.$$[event] || icaro({ count: 0 });
        RACCOON.$$[event].listen(fn || this.fnListen);
    }
    $unlisten(event, fn) {
        if (!event || !RACCOON.$$[event]) return;
        RACCOON.$$[event].unlisten(fn || this.fnListen);
        RACCOON.$$[event] = undefined;
    }
    $fire(event, value) {
        if (!event) return;
        RACCOON.$$[event] = RACCOON.$$[event] || icaro({ count: 0 });
        RACCOON.$$[event].value = value;
        ++RACCOON.$$[event].count;
    }
...
class CRACCOON {
    ...
    async createComponent(comp, props = {}) {
        ...
        if (typeof comp === 'string') {
            ...
            let url = `${urlRACCOON.replace('js/lit-raccoon.js', '')}components/${comp}/${comp}.js`;
            await import(url);
...
globalThis.RACCOON = new CRACCOON();
